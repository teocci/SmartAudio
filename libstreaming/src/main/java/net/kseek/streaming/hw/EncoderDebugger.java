...
import net.kseek.streaming.hw.CodecManager.Codec;
...
public class EncoderDebugger
{
    ...
    private void debug()
    {
        ...
        Log.e(TAG, "checkTestNeeded() = " + checkTestNeeded());
        if (!checkTestNeeded()) {
            String resolution = mWidth + "x" + mHeight + "-";

            boolean success = settings.getBoolean(PREF_PREFIX + resolution + "success", false);

            mB64PPS = settings.getString(PREF_PREFIX + resolution + "pps", "");
            mB64SPS = settings.getString(PREF_PREFIX + resolution + "sps", "");

            if (!success && (mB64PPS.equals("") || mB64SPS.equals(""))) {
                throw new RuntimeException("Phone not supported with this resolution (" + mWidth + "x" + mHeight + ")");
            }

            mNV21.setSize(mWidth, mHeight);
            mNV21.setSliceHeigth(settings.getInt(PREF_PREFIX + resolution + "sliceHeight", 0));
            mNV21.setStride(settings.getInt(PREF_PREFIX + resolution + "stride", 0));
            mNV21.setYPadding(settings.getInt(PREF_PREFIX + resolution + "padding", 0));
            mNV21.setPlanar(settings.getBoolean(PREF_PREFIX + resolution + "planar", false));
            mNV21.setColorPanesReversed(settings.getBoolean(PREF_PREFIX + resolution + "reversed", false));
            mEncoderName = settings.getString(PREF_PREFIX + resolution + "encoderName", "");
            mEncoderColorFormat = settings.getInt(PREF_PREFIX + resolution + "colorFormat", 0);

            return;
        }

        if (VERBOSE) Log.e(TAG, ">>>> Testing the phone for resolution " + mWidth + "x" + mHeight);
        ...
        for (Codec encoder : encoders) {
            for (Integer eColorFormat : encoder.formats) {
                reset();
                mEncoderName = encoder.name;
                mEncoderColorFormat = eColorFormat;

                if (VERBOSE)
                    Log.v(TAG, ">> Test " + (n++) + "/" + count + ": " + mEncoderName +
                            " with color format " + mEncoderColorFormat + " at " + mWidth + "x" + mHeight);
                ...
                try {
                    ...
                    for (Codec decoder : decoders) {
                        if (decoded) break;
                        for (Integer dColorFormat : decoder.formats) {
                            if (decoded) break;
                            mDecoderName = decoder.name;
                            mDecoderColorFormat = dColorFormat;
                            try {
                                configureDecoder();
                            } catch (Exception e) {
                                if (VERBOSE)
    ...
    private boolean checkTestNeeded()
    {
        ...
        if (settings.contains(PREF_PREFIX + resolution + "lastSdk")) {
            int lastSdk = settings.getInt(PREF_PREFIX + resolution + "lastSdk", 0);
            int lastVersion = settings.getInt(PREF_PREFIX + resolution + "lastVersion", 0);
...
//		}
    ...
    /**
     * @param withPrefix If set to true, the decoder will be fed with NALs preceded with 0x00000001.
